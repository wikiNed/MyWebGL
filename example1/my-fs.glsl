...
void tile_main()
{
    ...
    vec4 baseColorWithAlpha = SRGBtoLINEAR4(tile_diffuse_final(texture2D(u_baseColorTexture, custom_v_texcoord_0), tile_diffuse));
...
float light_width = 5.0;
vec3 light_color = vec3(1.0,1.0,1.0);
vec3 base_color = vec3(0.016,0.166,0.317);
vec3 top_color = vec3(0.023,0.478,0.694);
float light_limit = 1.0;
vec3 getBaseColor(vec3 base_color,vec3 top_color,float instance_height,float y){
    if(instance_height<0.1){
        return vec3(0.016,0.166,0.317);
    }
    return base_color+(top_color-base_color)*smoothstep(0.8*instance_height,instance_height,y);
}
float getLightIntensity(float instance_height,float cur_height_ratio,float fragY,float lightWidth){
    float cur_height=instance_height*cur_height_ratio;
    if(instance_height<0.1){
        return 0.0;
    }
    return clamp(1.0-abs(cur_height-fragY)/lightWidth,0.0,1.0);
}
vec2 transformTex(vec2 texcoord){
    mat2 tex_rs = mat2(v_tex_trs_0[0], v_tex_trs_0[1],
    v_tex_trs_1[0], v_tex_trs_1[1]);
    return v_tex_trs_2 + tex_rs * fract(texcoord);
}
void main(){
    custom_v_texcoord_0 = transformTex(v_texcoord_0);
    cesium_origin_main();
    gl_FragColor = vec4(getBaseColor(base_color,top_color,v_height,v_fragmentY),0.1);
    float intensity = getLightIntensity(v_height,u_height_ratio,v_fragmentY,light_width);
    gl_FragColor += vec4(light_color*light_limit*intensity,0.1);
}
